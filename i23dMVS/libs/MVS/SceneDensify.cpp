...
bool DepthMapsData::SelectViews(IndexArr& images, IndexArr& imagesMap, IndexArr& neighborsMap)
{
	...
	typedef I23D::cList<TypeGeneral::REAL, const TypeGeneral::REAL&, 0> EnergyCostArr;
...
bool DepthMapsData::SelectViews(DepthData& depthData)
{
	...
	//printf("fMinAngle: %lf ,fMaxAngle : %lf ,neighbors size: %d\n",fMinAngle,fMaxAngle,depthData.neighbors.GetSize());
...
bool DepthMapsData::InitViews(DepthData& depthData, uint32_t idxNeighbor, uint32_t numNeighbors)
{
	...
	if (idxNeighbor != NO_ID && numNeighbors < 2) {
		...
		//imageTrg.idx = neighbor.idx.ID;
		//printf("%d %d\n",idxImage,imageTrg.idx);
		...
		FOREACH(idx, depthData.neighbors) {
			...
			imageTrg.pImageData = &scene.images[neighbor.idx.ID];
			//imageTrg.idx = neighbor.idx.ID;
			//printf("%d %d\n",neighbor.idx.ID,imageTrg.pImageData->platformID);
			imageTrg.scale = neighbor.idx.scale;
			imageTrg.camera = imageTrg.pImageData->camera;
			imageTrg.pImageData->image.toGray(imageTrg.image, cv::COLOR_BGR2GRAY, true);
			if (imageTrg.ScaleImage(imageTrg.image, imageTrg.image, imageTrg.scale))
...
bool DepthMapsData::EstimateDepthMap(uint32_t idxImage)
{
	...
	if (OPTDENSE::nMinViewsTrustPoint < 2) {
		...
		depthData.dMin *= 0.9f;
		depthData.dMax *= 1.1f;
		VERBOSE("image %u: dmin %f, dmax %f", idxImage, depthData.dMin, depthData.dMax);
	} else {
...
/*
	...
	float * h_confmap;//n*h*w
	float * h_depthmap;//n*h*w
	float * h_normalmap;//n*h*w*3
	int * neighbors;
	
	{
		...
		h_confmap=new float[height*width];
		h_depthmap=new float[height*width];
		h_normalmap=new float[3*height*width];
		neighbors=new int[depthData.images.GetSize()+1];
		neighbors[0]=1;
		...
		for (int i=0;i<height;i++)
			for (int j=0;j<width;j++){
				ImageRef x(j,i);
				h_depthmap[i*width+j]=depthData.depthMap(x);
				//printf("%lf\n",depthData.depthMap(x));
				h_confmap[i*width+j]=depthData.confMap(x);
				//printf("%lf\n",depthData.confMap(x));
				h_normalmap[(i*width+j)*3]=depthData.normalMap(x).x;
				h_normalmap[(i*width+j)*3+1]=depthData.normalMap(x).y;
				h_normalmap[(i*width+j)*3+2]=depthData.normalMap(x).z;
			}
		
		//printf("\n%d",idxImage);
		FOREACH(idx, depthData.images) {
			DepthData::ViewData& imageTrg = depthData.images[idx];
			neighbors[neighbors[0]++]=imageTrg.idx;
			//printf(" %d",imageTrg.idx);
		}
		//printf("\n");
		//printf("%d %d\n",idxImage,neighbors[0]);
		//printf("%d %d\n",depthData.neighbors.GetSize(),depthData.images.GetSize());
		
		//SendRefData(idxImage,h_depthmap,h_confmap,h_normalmap);

		
		EstimateDepthMap_GPU(idxImage,depthData.dMin,depthData.dMax,h_depthmap,h_confmap,h_normalmap,neighbors);

		
		//CarryOutData(idxImage,h_depthmap,h_confmap,h_normalmap);
		for (int i=0;i<height;i++)
			for (int j=0;j<width;j++){
				ImageRef x(j,i);
				float &depth=depthData.depthMap(x);
				depth=h_depthmap[i*width+j];
				//printf("%lf\n",depth);
				float &conf=depthData.confMap(x);
				conf=h_confmap[i*width+j];
				//printf("%lf\n",conf);
				float &normalx=depthData.normalMap(x).x;
				normalx=h_normalmap[(i*width+j)*3+0];
				float &normaly=depthData.normalMap(x).y;
				normaly=h_normalmap[(i*width+j)*3+1];
				float &normalz=depthData.normalMap(x).z;
				normalz=h_normalmap[(i*width+j)*3+2];
				//printf("%lf %lf\n",depth,depthData.depthMap(x));
			}
...
//		#endif
	...
	if (bAdjust) {
		...
		const float thDepthDiffStrict(OPTDENSE::fDepthDiffThreshold);
		const unsigned nMinGoodViewsProc(80), nMinGoodViewsDeltaProc(40);
...
bool DepthMapsData::EstimateScaleMap() {
    FOREACH(idxImage, scene.images) {
        DepthData& depthData = arrDepthData[idxImage];
        if (!depthData.IsValid())
            continue;
		if (depthData.IsEmpty() && depthData.IncRef(ComposeDepthFilePath(idxImage, "dmap")) == 0)
			return false;
		ASSERT(!depthData.IsEmpty());

        const Image8U::Size sizeMap(depthData.depthMap.size());
        ScaleMap &scaleMap = depthData.scaleMap;
        scaleMap.create(sizeMap);
        scaleMap.memset(0);
		...
		for (int i=0; i<sizeMap.height; ++i) {
			for (int j=0; j<sizeMap.width; ++j) {
				...
				if (depth == 0)
					continue;
                PointCloud::Point point = imageData.camera.TransformPointI2W(Point3(Point2f(x),depth));
                ImageRef xCs[] = {
                        ImageRef(j-1,i-1), ImageRef(j-1,i), ImageRef(j-1,i+1),
                        ImageRef(j,i-1),                    ImageRef(j,i+1),
                        ImageRef(j+1,i-1), ImageRef(j+1,i), ImageRef(j+1,i+1)};
                int nNeighbor = 0;
                float& scale = scaleMap(x);
                for (uint32_t ii = 0; ii < 8; ++ii) {
                    ImageRef xC = xCs[ii];
                    if (!depthData.depthMap.isInside(xC))
                        continue;
                    const Depth depthC(depthData.depthMap(xC));
                    if (depthC == 0)
                        continue;
                    PointCloud::Point pointC = imageData.camera.TransformPointI2W(Point3(Point2f(xC),depthC));
                    scale += cv::norm(point - pointC);
                    nNeighbor += 1;
                }
                if (nNeighbor != 0)
                    scale /= static_cast<float>(nNeighbor);
                else { // FootPrint
                    ImageRef xC = ImageRef(j-1,i);
                    PointCloud::Point pointC = imageData.camera.TransformPointI2W(Point3(Point2f(xC),depth));
                    scale = cv::norm(point - pointC);
                }
            }
        }
...
typedef I23D::cList<Proj,const Proj&,0,4,uint32_t> ProjArr;
typedef I23D::cList<ProjArr,const ProjArr&,1,65536> ProjsArr;
...
void DepthMapsData::FuseDepthMaps(PointCloud& pointcloud, bool bEstimateNormal)
{
	...
	FOREACH(i, scene.images) {
		...
		if (!depthData.IsValid())
			continue;
		if (depthData.IsEmpty() && depthData.IncRef(ComposeDepthFilePath(i, "dmap")) == 0)
			return;
		ASSERT(!depthData.IsEmpty());
...
//	typedef I23D::cList<DepthIndex,const DepthIndex&,1> DepthIndexArr;
//	DepthIndexArr arrDepthIdx(scene.images.GetSize());
	...
	printf("the nPointsEstimate is %d\n",nPointsEstimate);
	
/*	
	//send connect data to gpu
	connections_data=new int[connections.GetSize()];
	int N=0;
	FOREACHPTR(pConnection, connections) {
		connections_data[N++]=pConnection->idx;
	}

	/////////////////////////////////////////////////////////////
	//make all the data to a format and send the data to gpu
	//size_data=new int[N*2];
	Image8U::Size sizeMap(arrDepthData[0].depthMap.size());
	int height=sizeMap.height;
	int width=sizeMap.width;

	h_images_K=new float[N*9];
	h_images_R=new float[N*9];
	h_images_C=new float[N*3];

	h_depthmap =new float[N*sizeMap.height*sizeMap.width];
	h_confmap  =new float[N*sizeMap.height*sizeMap.width];
	h_neighbors=new int[N*N];

	h_neighbors_begin=new int[N];
	h_neighbors_end=new int[N];

	h_points=new float[nPointsEstimate*3];
	h_weight=new float[nPointsEstimate*(N+1)];
	h_view=new int[nPointsEstimate*(N+1)];
	h_pointsinfo=new int[nPointsEstimate*2];

	///////////////////////////////////////////////////////////
	for (int idxImage=0;idxImage<N;idxImage++) {
		DepthData& depthData(arrDepthData[idxImage]);
		Image8U::Size sizeMap(depthData.depthMap.size());
		Image& imageData = *depthData.images.First().pImageData;	
		Image& imageData2 = scene.images[idxImage];

		
		//copy data to h_ref
		for (int i=0;i<3;i++){

			h_images_C[idxImage*3+i]=imageData2.camera.C[i]; 

			for (int j=0;j<3;j++){

				h_images_K[idxImage*9+i*3+j]=imageData2.camera.K(i,j);
				h_images_R[idxImage*9+i*3+j]=imageData2.camera.R[i*3+j];
			}
		}

		if (N==0)
			h_neighbors_begin[idxImage]=0;
		else h_neighbors_begin[idxImage]=h_neighbors_end[idxImage-1];
		
		h_neighbors_end[idxImage]=h_neighbors_begin[idxImage]+depthData.neighbors.GetSize();
				

		for (int i=0;i<sizeMap.height;i++){
			for (int j=0;j<sizeMap.width;j++){
				int pos=idxImage*sizeMap.width*sizeMap.height+i*sizeMap.width+j;
				ImageRef x(j,i);
				Depth depth(depthData.depthMap(x));
				h_depthmap[pos]=depth;
				h_confmap[pos] =depthData.confMap(x);
			}
		}
		int i_neighbors=0;
		FOREACHPTR(pNeighbor, depthData.neighbors) {
			uint32_t idxImageB(pNeighbor->idx.ID);
			h_neighbors[h_neighbors_begin[idxImage]+i_neighbors++]=idxImageB;
		}
	}
	...
	FuseDepthMaps_GPU(connections_data,
		N,
		height,
		width,
		h_images_K,
	...
	//////////////////////////////////////////////
	for (int i=0;i<h_pointsinfo[0];i+=3)
	{
		int pos=i;
		...
		point.x=h_points[pos];
		point.y=h_points[pos+1];
		point.z=h_points[pos+2];
		//if (point.y>100) printf("%d %.3lf %.3lf\n",i/3,point.y,h_points[1]);
		//printf("%.3lf %.3lf %.3lf\n",point.x,point.y,point.z);
		pos=pos/3+1;
		int view_size=h_pointsinfo[pos*2];
		int begin=h_pointsinfo[pos*2+1];
		for (int j=0;j<view_size;j++)
		{
			float weight=h_weight[begin+j];
			int view=h_view[begin+j];
			const uint32_t idx(views.InsertSort(view));
			weights.InsertAt(idx, weight);
		}
	}
		//printf("####################         %d\n",h_pointsinfo[0]/3);
	...
	free(h_neighbors_begin);
	free(h_neighbors_end);
...
//						arrDepthIdx[idxImage](x).idx = NO_ID;
	...
	//for (int i=0;i<(int)pointcloud.points.GetSize();i++)
	{
		//printf("%.3lf %.3lf %.3lf\n",pointcloud.points[i].x,pointcloud.points[i].y,pointcloud.points[i].z);
		//for (int j=0;j<(int)pointcloud.pointWeights[i].GetSize();j++) printf("%.3lf ",pointcloud.pointWeights[i][j]); printf("\n");
		//for (int j=0;j<(int)pointcloud.pointViews[i].GetSize();j++) printf("%d ",pointcloud.pointViews[i][j]); printf("\n");
...
//        pointcloud.ComputeScale();
	}

	//save the depthmap to png
	FOREACH(i, scene.images) {
		DepthData& depthData = arrDepthData[i];
		...
		ASSERT(!depthData.IsEmpty());
		ExportDepthMap(ComposeDepthFilePath(i, "png"), depthData.depthMap);
	}
...
struct DenseDepthMapData {
	Scene& scene;
	IndexArr images;
	IndexArr imagesMap; // maps global view indices to our list of views to be processed
	IndexArr neighborsMap;
	DepthMapsData detphMaps;
	volatile Thread::safe_t idxImage;
	I23D::EventQueue events; // internal events queue (processed by the working threads)
	Semaphore sem;
...
bool Scene::DenseReconstruction()
{
	...
	// prepare images for dense reconstruction (load if needed)
	{
		...
		//printf("-------------     %d\n",images.GetSize());
		data.imagesMap.Resize(images.GetSize()); // maps global view indices to our list of views to be processed
		#ifdef DENSE_USE_OPENMP
		bool bAbort(false);
		...
		FOREACH(idxImage, images) {
			...
			#endif
			{
				data.imagesMap[idxImage] = (uint32_t)data.images.GetSize();
	...
	// select images to be used for dense reconstruction
	{
		...
		for (int_t ID=0; ID<(int_t)data.images.GetSize(); ++ID) {
			const uint32_t idxImage((uint32_t)ID);
		#else
		FOREACH(idxImage, data.images) {
		#endif
			DepthData& depthData(data.detphMaps.arrDepthData[idxImage]);
			data.detphMaps.SelectViews(depthData);
		}
		// globally select a target view for each reference image
		if (OPTDENSE::nNumViews != 0 && !data.detphMaps.SelectViews(data.images, data.imagesMap, data.neighborsMap)) {
...
/*
	//startgpu 
	{
		timer.Start();
		//int * h_confmap;//n*h*w
		//int * h_depthmap;//n*h*w
		float * h_images;//n*h*w
		//int * h_normalmap;//n*h*w*3
		float * h_images_R;//n*9
		float * h_images_K;//n*9
		float * h_images_C;//n*3		

		const Image& imageData = data.scene.images[0];
		
		int width=imageData.width;
		int height=imageData.height;
		int n=data.images.GetSize();
		
		h_images=new float[n*width*height];
		h_images_K=new float[n*9];
		h_images_R=new float[n*9];
		h_images_C=new float[n*3];
		...
		for (int idx=0;idx<n;idx++) {
			Image& pImage=data.scene.images[idx];
			Image32F image;
			pImage.image.toGray(image,cv::COLOR_BGR2GRAY,true);
			
			for (int i=0;i<height;i++)
				for (int j=0;j<width;j++){
					h_images[idx*width*height+i*width+j]=image(i,j);
				}
			
			Image& imageData2 = data.scene.images[idx];
			
			for (int i=0;i<3;i++){

				h_images_C[idx*3+i]=imageData2.camera.C[i]; 

				for (int j=0;j<3;j++){

					h_images_K[idx*9+i*3+j]=imageData2.camera.K(i,j);
					h_images_R[idx*9+i*3+j]=imageData2.camera.R[i*3+j];
				}
			}
			
			//printf("%d\n",idx);
		}
		InitialAndSendImageData(n,height,width,h_images,h_images_R,h_images_K,h_images_C);
		free(h_images);
		free(h_images_K);
		free(h_images_R);
		free(h_images_C);
...
*/
	...
	//endgpu
	{
		//MemoryFree();
		//timer.Stop();
		//printf("\nEstimateDepthMaps ran in: %f msecs.\n", timer.Elapsed());
	}
	...
	if ((OPTDENSE::nOptimize & OPTDENSE::ADJUST_FILTER) != 0) {
		...
		if (nMaxThreads > 1) {
			// multi-thread execution
			cList<I23D::Thread> threads(MINF(nMaxThreads, (unsigned)data.images.GetSize()));
			FOREACHPTR(pThread, threads)
...
//printf("fuseDepthMaps ran in: %f msecs.\n", timer.Elapsed());
	...
	#endif
	//printf("%d %d %d %d\n",OPTDENSE::nEstimateColors,OPTDENSE::nEstimateNormals,pointcloud.colors.IsEmpty(),pointcloud.normals.IsEmpty());
	...
	//printf("%d %d\n",pointcloud.colors.GetSize(),pointcloud.normals.GetSize());
	/*
	for (int i=0;i<pointcloud.colors.GetSize();i++){
		printf("%u\n",pointcloud.colors[i]);
	}
	*/
	/*
	for (int i=0;i<pointcloud.normals.GetSize();i++){
		printf("%.3lf %.3lf %.3lf\n",pointcloud.normals[i].x,pointcloud.normals[i].y,pointcloud.normals[i].z);
	}
	*/
	...
	while (true) {
		...
		case EVT_OPTIMIZEDEPTHMAP: {
			...
			if (OPTDENSE::nOptimize & (OPTDENSE::REMOVE_SPECKLES)) {
				...
				VERBOSE("Depth-map %3u filtered: remove small segments (%s)", idx, TD_TIMER_GET_FMT().c_str());
			}
			if (g_nVerbosityLevel > 3)
				ExportDepthMap(ComposeDepthFilePath(idx, "remove_speckles.png"), depthData.depthMap);
			...
			if (g_nVerbosityLevel > 3)
				ExportDepthMap(ComposeDepthFilePath(idx, "fill_gap.png"), depthData.depthMap);
			// save depth-map
			data.events.AddEventFirst(new EVTSaveDepthMap(evtImage.idxImage));
			break; }
...
void Scene::DenseReconstructionFilter(void* pData)
{
	...
	while ((evt=data.events.GetEvent(0)) != NULL) {
		...
		case EVT_ADJUSTDEPTHMAP: {
			...
			data.sem.Wait();
			// load filtered maps
			if ((depthData.IsEmpty() && depthData.IncRef(ComposeDepthFilePath(idx, "dmap")) == 0) ||
