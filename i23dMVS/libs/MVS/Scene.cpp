...
bool Scene::Combine(const Scene& otherScene) {
	// combine images
	int oriPlatformSize = platforms.GetSize();
	int oriImageSize = images.GetSize();
	images.Join(otherScene.images);
	for (unsigned i = oriImageSize; i < images.GetSize(); ++i) {
		images[i].platformID += oriPlatformSize;
	}
	// combine platform
	platforms.Join(otherScene.platforms);

	nCalibratedImages += otherScene.nCalibratedImages;
	return true;
} // Combine
/*----------------------------------------------------------------*/
...
bool Scene::SelectNeighborViews(uint32_t ID, IndexArr& points, unsigned nMinViews, unsigned nMinPointViews, float fOptimAngle)
{
	...
	//printf("pointsA size : %d ,pointsB size : %d\n",pointsA.GetSize(),pointsB.GetSize());
	...
	FOREACH(IDB, images) {
		...
		//printf("boundsA x : %d ,boundsA y : %d\n",(int)boundsA.x,(int)boundsA.y);
		...
		//printf("neighbor.idx.area : %lf ,neighbor.idx.scale : %lf ,neighbor.idx.angle : %lf\n",neighbor.idx.area,neighbor.idx.scale,neighbor.idx.angle);
	...
	//printf("points size is %d ,neighbors size is %d , the ID is %d\n",points.GetSize(),neighbors.GetSize(),ID);
...
bool Scene::FilterNeighborViews(ViewScoreArr& neighbors, float fMinArea, float fMinScale, float fMaxScale, float fMinAngle, float fMaxAngle, unsigned nMaxViews)
{
	...
	//printf("neighbors size is %d,nMaxViews is %d\n",neighbors.GetSize(),nMaxViews);
